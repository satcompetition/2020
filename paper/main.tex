...
With standardized inputformats, readily-available APIs for incrementalapplications,
and certified proof logging andchecking capabilities, applications of SAT solver technology
...
for maintaining and furthering this progress. In particular, 

WILL CONTINUE FROM HERE -MATTI
...
\solver{Painless-MCOMSPS-STR} integrates the solver \solver{MapleCOMSPS} in the \solver{Painless} parallelization framework~\cite{Frioux:2017:Painless}~\cite{Liang:2017:Maplecomsps}. 
The authors submitted a \solver{32 core} and a \solver{64 core} variant and were awared three prices: first place in the Main track, first place in the Sat track and second in the Unsat track. 
...
\solver{Painless} uses a generic interface to integrate a solver and abstracts away the implementation details of parallelism and concurrenct data-structures, such that implementations in \solver{Painless} boil down to implementing parallelization and clause sharing strategies. 
...
The aggragate that approaches the root of the binary tree is then broadcast to all solvers~\cite{}. 
...
\solver{TopoSAT~2} uses lockfree clause-exchange for solvers running on the same maching, and MPI to share clauses between machines~\cite{ManySat,Proceedings}. 
...
\solver{Slime} is build from \solver{MapleLCMDistChronoBT} and was first submitted as a sequential solver to SAT Race~2019 with a new phase selection heuristic~\cite{SlimeSATRace2019}. 
