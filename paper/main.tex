...
Each solver run in this track was on a m4.16xlarge machine from Amazon Web Services.
...
$S_{1},\dots,S_{n}$ for each solver $S$. The \emph{similarity} of two solvers $S$
...
dendrogram. A similar visualization was used for the results of the SAT Race 2015 \cite{Balyo:2015:SATRace}.
...
changes they made either result in very similar behavior or do not have a
...
Close to them are the three \solver{CMS} versions. They differ in their implementation of stochastic local search
...
the lowest average similarity to all other solvers in the top 30. This hints at
its importance for the use in an optimal portfolio.
% Average similarity
...
highest ranked solver (\solver{Kissat-sat}) with the lowest ranked solver,
results in a rank correlation of $\rho = 0.7602$. Moving \solver{Kissat-sat} to
the bottom of the ranking and moving every other solver up one rank gives a
...
The individual removal of all other benchmarks families results in a rank correlation over $0.99$.
