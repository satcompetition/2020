...
Some interesting results were observed in the 2020 SAT Competition.
Four Main track solvers (\solver{Kissat-sat}, \solver{Relaxed-newTech}, \solver{CaDiCaL-alliup-trial}, and \solver{CMS-ccnr-lsids}) 
performed significantly better compared to the others, see Figure~\ref{main:all} and Table~\ref{tab:results}.
These four solvers all have a different code base. In recent competitions, top solvers were much closer together.
The main reason for this difference is the enormous gap with the other solvers on satisfiable benchmarks, 
...
and variable-move-to-front). The solvers performance was much closer on unsatisfiable formulas,
see Figure~\ref{main:unsat}. Only \solver{Kissat-unsat} performed significantly better compared to all
other participants. It is therefore not surprising that the VBS is reasonably close to \solver{Kissat-unsat}.
