...
In the following, we report on the best scores achieved by tuples and triples of solvers in terms of their VBS's PAR-2 score. 

\subsubsection{Best Tuples}

The best scoring solver tuple is \solver{Kissat} and \solver{CryptoMiniSat-CCAnr}. 
The VBS of those two solvers achieved a PAR-2 score of $3028$. Other combinations of \solver{Kissat} in its default configuration and \solver{CryptoMinisat} variants achieved runner-up scores as can be seen in Table~\ref{tab:best-tuples}. 
Further best performing tuples can be created from combinations of \solver{CaDiCaL-alluip} and \solver{CryptoMiniSat-CCAnr} or \solver{Kissat} and \solver{Relaxed-newTech}. 
...
\multirow{3}{*}{\solver{Kissat}} & \solver{CryptoMiniSat-CCAnr} & $3028.0$\\
 & \solver{CryptoMiniSat-CCAnr lsids} & $3052.8$\\
 & \solver{CryptoMiniSat-Walksat} & $3127.7$\\
\hline
\solver{CaDiCaL-alluip} & \multirow{2}{*}{\solver{CryptoMiniSat-CCAnr}} & $3128.0$\\
\solver{CaDiCaL-alluip-trail} &  & $3128.0$\\
\solver{CaDiCaL-alluip} & \multirow{2}{*}{\solver{CryptoMiniSat-Walksat}} & $3152.8$\\
\solver{CaDiCaL-alluip-trail} &  & $3152.9$\\
\hline
\solver{Kissat} & \multirow{2}{*}{\solver{Relaxed-newTech}} & $3177.7$\\
\solver{Kissat-unsat} &  & $3177.8$
\end{tabular}
\caption{The $9$ best performing tuples of solvers in terms of the PAR-2 score of their VBS.}
\label{tab:best-tuples}
\end{table}

\subsubsection{Best Triples}

The best triple of solvers consists of \solver{Kissat}, \solver{CryptoMiniSat-CCAnr lsids} and \solver{f2trc-DL}. 
In combination their VBS achieves a PAR-2 score of $2827.7$, thus getting very close to the score or the overall VBS. 
Ohter combinations of \solver{Kissat} and \solver{CryptoMiniSat-CCAnr} with \solver{Scavel01} or \solver{DurianSat} achieved runner-up scores as can be seen in Table~\ref{tab:best-triples}. 
...
\hline\arrayrulecolor{lightgray}
\solver{Kissat} & \solver{CryptoMiniSat-CCAnr lsids} & \solver{f2trc-DL} & $2827.7$\\
\hline
\multirow{3}{*}{\solver{Kissat}} & \multirow{3}{*}{\solver{CryptoMiniSat-CCAnr}} & \solver{Scavel01} & $2852.7$\\
 &  & \solver{f2trc-DL} & $2852.8$\\
 &  & \solver{DurianSat} & $2852.8$\\
\hline
\solver{Kissat} & \solver{CryptoMiniSat-CCAnr lsids} & \solver{Scavel01}& $2877.6$\\
...
\end{tabular}
\caption{The $5$ best performing triples of solvers in terms of the PAR-2 score of their VBS.}
