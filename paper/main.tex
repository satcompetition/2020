...
\centering
\begin{tabular}{cccl}
\bf Place & \bf PAR-2 Score & \bf Num. Solved & \bf Solver \\
\hline
...
\bf Place & \bf PAR-2 Score & \bf Num. Solved & \bf Solver \\
\hline
 1 & 3316.6 & 283 & \solbert{Painless-MCOMSPS-STR-32} \\
(1)& 3714.7 & 271 & \solver{Painless-MCOMSPS-STR-64} \\
...
\bf Place & \bf PAR-2 Score & \bf Num. Solved & \bf Solver \\
\hline
 1 & 2853.7 & 153 & \solbert{Painless-MCOMSPS-STR-32} \\
 2 & 2913.6 & 154 & \solbert{Painless-Maple-v1} \\
(2)& 3082.7 & 151 & \solver{Painless-Maple-v2} \\
-- & 3196.9 & 148 & \solver{Painless-MCOMSPS-STR-64} \\
...
\hline
 ...
 2 & 3779.5 & 130 & \solbert{Painless-MCOMSPS-STR-32} \\
...
\hline
Soos, Cai, Devriendt, Gocht, Shaw, Meel & \solver{CryptoMiniSat 5} & 3 \\
\stack{Biere, Fleury}~ & \solver{CaDiCaL 2020} & 1 \\
Chen & \solver{abcdsat-i20} & 1 \\
Manthey & \solver{Riss 7.1.2} & 1 
\end{tabularx}
...
\bf Team & \bf Solver & \bf M & \bf U & \bf S \\
\hline\arrayrulecolor{lightgray}
...
\multirow{2}{*}{Tchinda, Djamegni}
 & \solver{painlessmaplevone} & --& --& 2 \\
 & \solver{painlessmaplevtwo} & --& --& 2 \\
\hline
...
\subsubsection{Painless MCOMSPS STR} 
...
The authors submitted a \solver{32 core} and a \solver{64 core} variant and won three awards: first place in the Main track, best score on satisfiable and second best score on unsatisfiable instances. 
The \solver{32 core} variant achieved slightly better scores than the \solver{64 core} variant. 
The \solver{32 core} variant performed much better, such that the runner-up solvers on satisfiable and unsatisfiable instances outperformed the \solver{64 core} variant. 
...
Each table shows the first ten solvers sorted according to the respective metric.\todo{Maybe move classic VBS also to here for comparison}}
...
3 & 1666 & 272 & \{ \solver{Kissat-unsat}, \solver{Kissat-sat}, \phantom{\{ }\solver{CMS-ccnr-lsids} \} \\
4 & 1250 & 262 & \{ \solver{Kissat-unsat}, \solver{Kissat-sat}, \phantom{\{ }\solver{CMS-ccnr-lsids}, \solver{Maple-alluip-trail} \}\\ 
5 & 1000 & 253 & \{ \solver{Kissat-unsat}, \solver{Kissat-sat}, \phantom{\{ }\solver{CMS-ccnr-lsids}, \solver{Maple-alluip-trail}, \phantom{\{ }\solver{CaDiCaL-alluip} \}\\
\end{tabulary}
...
The PAR-2 score of the VBS of all $48$ submitted solvers in the Main track is $2431.4$, 
which is much better than that of the single best solver  \solver{Kissat-sat} with a PAR-2 score of $3926.2$. 
We also calculated the PAR-2 score of the VBS of small subsets of the $48$ submitted solvers. 
In Table~\ref{tab:tuples}, we report on the best performing sets of solvers of fixed size $k \in \{1, \dots, 7\}$. 


\subsection{Score per Instance Family} 

\todo{Interestingly, the second (Relaxed-Maple-newTech) and the third (CryptoMinisat-lsids) ranked solver both outperform the winning solver (Kissat-sat) when we project only on the instance families ``antibandwidth'' or ``timetable''. }
