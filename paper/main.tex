...
To the Main track, the solver was submitted in two variants, with and without LSIDS~\cite{}, which is a phase selection heuristic similar to VSIDS and if active it is used whenever backtracking is chronological~\cite{}. 
On top of that, as in CaDiCal (and now Kissat), CryptoMiniSat is periodically switching between polarity saving and stable phases. 
...
This modified version of last years CaDiCal was submitted with the two new methods \emph{Trail Saving}\cite{} and \emph{Stable AllUIP}. 
...
With \emph{Stable AllUip}, the solver continues resolution beyond the 1st Unit Implication Point (1-UIP) during clause-learning as long that does not increase the LBD value of the learnt clause and then keeps that clause only if it has a smaller size than then 1-UIP clause. 
The authors came up with an additional heuristic to limit the amount of attempted AllUip learning attempts, if the fraction of unsuccessful alluip learning attempts gets to high, which they check and adapt on every restart. 
...
The \emph{Relaxed} fork of MapleLCMDistCBT-DL already participated in SAT Race 2019.
...
In the new versions of the Relaxed family of Maple solvers, they probabilistically switch between several modes of phase selection, and some of these modes reuse the maximal partially satisfying assignments as determined by the local search solver.

For their successful newTech variant of their fork, they count the number of variable-occurrences in unsat clauses for each flip in the local search solver. 
...
The F2TRC forks aim at introducing determinism in winning solvers of previous competition, e.g., by replacing time-based intervals for LRB-VSIDS switches by a heuristic based on the conflict-count~\cite{}. 

Another modification aims at improving three-tier clause management based on the three tiers core, tier2 and local as it was introduced by Chanseok Oh.
...
In order to prevent tier2 from starving, they replaced the conflict limit for tier2 reduction by a size limit, then only half of the most recently inactive clauses is moved to local tier. 
