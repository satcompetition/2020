...
This left us with 46 configurations of 22 solvers, inluding only one Glucose hack.
...
Examples for applications of incremental SAT solving are counterexample-guided eastraction refinement (CEGAR) based approaches, e.g., for Bounded Model Checking~\cite{Strichman:2005:ARBMC}, SAT-based planning~\cite{Balyo:2017:IncrPlanning} and satisfiability modulo theories (SMT) solvers~\cite{Brummayer:2009:Boolector}. 
...
the fortchoming invocations of the solver.
...
In contrast to the Main track, proof logging for unsatisfiable instances is not required in the Parallel track. 
...
A key aim of benchmark selection is to ensure that the number of benchmarks included in the benchmark set from different problem domains 
is balanced across the problem domains.
With these considerations in mind, to construct the benchmark set, we executed the following steps of \emph{rule-based randomization}. 
