...
remove $84$ benchmarks that have not been solved by any solver. For the rest of
the benchmarks we use the PAR-2 score,
% based on the \num{5000}-second timeout
i.\,e., for each solver we set a score of \num{10000} for unsolved instances.
This leaves us with $316$ floats
$S_{1},\dots,S_{n}$ for each solver $S$. The \emph{similarity} of two solvers $A$
...
% \label{eq-similarity}
\[
	similarity(A, B) =\quad 1 - \frac{\sum{|A_{i} - B_{i}|}}{316 \cdot \num{10000}}
\]
% \end{equation}
...
a similarity above \num{0.999}. Thus the two solvers are joined low in the
dendrogram.
A similar visualization was used to visualize the results of the SAT Race \cite{Balyo:2015:SATRace}.

One big cluster we can identify are the \solver{Maple}-decedents to the right,
all of which are modifications of the winners of the SAT Competition 2018 and
the SAT Race 2019.
With the exception of \solver{mergesat} and
maybe \solver{DurianSat} we observe a high similarity within the cluster.
% * 0.9645726159423101, 'exp_V_MLD_CBT_DL', 'Scavel' second highest similarity of all (not 2 configurations of the same solver)
% ** both based on MapleLCMDistChronoBT-dl-v2.2
% ** suggesting that the changes they made are either very similar or both do not have a high impact on the performance of the base solver in this test set.

% The two configurations of the \solver{Relaxed} by \emph{Zhang} and \emph{Cai}
use the same code base as a lot of the solvers in the Maple-Cluster but their
overall performance is better and closer to the CMS-Cluster.

The three CMS versions differ in their implementation of stochastic local search
(SLS) but overall have a similar performance.

The last cluster to the left of the heat map is comprised of the solvers
originally written by Biere. The three configurations of \solver{Kissat} form a
...
\solver{CaDiCaL}. The new version CaDiCaL-2020 has a worse overall performance
...
We use random sampling to select a subset of benchmarks used in the Main track.
We start with $316$ benchmarks that have been solved by at least one solver in
the Main track and remove a number of benchmarks randomly.
...
If even relatively small random samples result in a good estimate, we can draw a
...
To determine how similar the estimate is to the original ranking, we calculate
the \emph{Spearman's rank correlation coefficient} of the two rankings. For each
...
modification as before $n/2$ times to switch every two adjacent solvers in rank
still gives a value of $\rho = 0.9974$. On the other hand, switching the highest
ranked solver (\solver{Kissat-sat}) with the lowest ranked solver, results in a
rank correlation of $\rho = 0.7602$. Moving \solver{Kissat-sat} to the bottom of
the ranking and moving every other solver up one rank gives a higher
$\rho = 0.8776$. Doing the same to all three \solver{Kissat} configurations
(with ranks 1, 2 and 11, now 46, 47 and 48) results in $\rho = 0.6839$.
...
\begin{figure}[h]
  ...
  \caption{Mean and standard deviation of rank correlation under removal of random instances, 50 samples for each size}
\end{figure}
...
Furthermore, removing fewer than $200$ ($63\%$) benchmarks, results in an average rank correlation above $0.96$

This suggest that the influence of the random selection in Algorithm~\ref{algo:select} on the solver ranking is limited.
% \ref{sec:byob}

The collected data cannot show, whether all of the benchmarks originally
submitted by the solver authors, have a systematic bias. Since each newly
submitted benchmark family originates from a different domain and is often the
result of current research, we can assume that the submitted families together
are representative. Figure~\ref{fig:sampleFamily} shows the rank correlation
coefficient resulting from removing a complete benchmark family instead of a
random subset. As expected removing a non random subset can have a higher impact
on the ranking even if it is small. Removing the 13 benchmarks from the
\emph{hgen}-family results in a (still high) rank correlation of $0.9895$. Additionally, the ranking of the top five solvers stays the same.
