...
#import data
data<-read.csv(file, header=TRUE, row.names = 1, sep = ",", check.names=F)

#calculate distance matrix from correlation matrix

#compute clusterint
#hc<-hclust(data_d,"average")
#plot clustering
#plot(hc)

#save the data as a scaled matrix
datascaled <-as.matrix(scale(data))
datam <-as.matrix(data)
#stop(colnames(data))
#stop(colnames(datam))
pdffile = noquote(strsplit(file, split="\\.")[[1]][1])
pdf(paste("./",pdffile,".pdf", sep=""), height=13, width=10)
#plot data as a clustered heatmap
#colorpallete = brewer.pal(100,"YlOrRd") #yellow -> red transition
colorpallete = colorRampPalette(brewer.pal(9,"YlOrRd"))(100) #yellow -> red transition
#colorpallete = brewer.pal(9,"Greys") #black->white transition

#heatmap(datam, Colv=T, Rowv=T, scale = 'none',col = colorpallete ,margins=c(12,12))
myheatmap(datam,trace="none",revC=FALSE, Colv=TRUE, Rowv=TRUE, scale = 'none',col = colorpallete, cexRow=1.2,cexCol=1.2,margins=c(12,12),key=TRUE,dendrogram = "column",lmat=rbind( c(0, 3), c(2,1), c(0,4) ), lhei=c(1, 5, 1 ),lwid=c(0.1, 2 )) #,density.info='none',keysize=1,
dev.off()
}

# copied from heatmap.2
myheatmap<-function (x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE, 
    ...
    lwid = NULL, ...) 
{
    ...
    if (key) {
        ...
        if (density.info == "density") {
            ...
            title("Color Key and Density Plot")
            par(cex = 0.5)
